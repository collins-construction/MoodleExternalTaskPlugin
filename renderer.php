...
/**
 * External task question renderer class.
 *
 * @package    qtype
...
/**
 * Generates the output for externaltask questions.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_externaltask_renderer extends qtype_renderer {
    ...
    public function files_input(question_attempt $qa, $numallowed,
        ...
        if (!empty($qa->get_question()->filetypeslist)) {
            $text = html_writer::tag('p', get_string('acceptedfiletypes', 'qtype_externaltask'));
    ...
    public function manual_comment(question_attempt $qa, question_display_options $options) {
        ...
        return html_writer::nonempty_tag('div', $question->format_text(
                $question->graderinfo, $question->graderinfo, $qa, 'qtype_externaltask',
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_externaltask_format_renderer_base extends plugin_renderer_base {
    /**
...
/**
 * An externaltask format renderer for externaltasks where the student should not enter
 * any inline response.
 *
 * @copyright  2013 Binghamton University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_externaltask_format_noinline_renderer extends plugin_renderer_base {

    protected function class_name() {
        return 'qtype_externaltask_noinline';
    }
...
/**
 * An externaltask format renderer for externaltasks where the student should use the HTML
 * editor without the file picker.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_externaltask_format_editor_renderer extends plugin_renderer_base {
    protected function class_name() {
        return 'qtype_externaltask_editor';
    }

    public function response_area_read_only($name, $qa, $step, $lines, $context) {
        return html_writer::tag('div', $this->prepare_response($name, $qa, $step, $context),
                array('class' => $this->class_name() . ' qtype_externaltask_response readonly'));
    }
    ...
    public function response_area_input($name, $qa, $step, $lines, $context) {
        ...
        $output .= html_writer::start_tag('div', array('class' =>
                $this->class_name() . ' qtype_externaltask_response'));
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_externaltask_format_editorfilepicker_renderer extends qtype_externaltask_format_editor_renderer {
    protected function class_name() {
        return 'qtype_externaltask_editorfilepicker';
    }
    ...
    protected function specific_filepicker_options($acceptedtypes, $draftitemid, $context) {
        debugging('qtype_externaltask_format_editorfilepicker_renderer::specific_filepicker_options() is deprecated, ' .
...
/**
 * An externaltask format renderer for externaltasks where the student should use a plain
 * input box, but with a normal, proportional font.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_externaltask_format_plain_renderer extends plugin_renderer_base {
    /**
     * @return string the HTML for the textarea.
     */
    protected function textarea($response, $lines, $attributes, $question = null) {

        global $COURSE, $USER;

        $attributes['class'] = $this->class_name() . ' qtype_externaltask_response';
        $attributes['rows'] = $lines;
        $attributes['cols'] = 60;
        $attributes['hidden'] = true;

        $taskcomplete = $question->get_question()->taskcomplete;

        $checked = s($response) ? 'checked' : '';
        ...
        $disabled = !$isStudent ? 'disabled' : '';
            ...
            $check = '<p><input type="hidden" name="' . $attributes['name'] . '" value=""></p>';
            $check .= '<p><input type="checkbox" ' . $checked . ' name="' . $attributes['name'] . '" value="' . $taskcomplete . '"> ' . $taskcomplete . '</p>';
        } else {
            $disabled = 'disabled';
            $check = '<p><input type="checkbox" ' . $checked . ' ' . $disabled . ' name="readonly" value="readonly"> ' . $taskcomplete . '</p>';
        }

        return html_writer::tag('textarea', s($response), $attributes) . $check;
    }

    protected function class_name() {
        return 'qtype_externaltask_plain';
    }

    public function response_area_read_only($name, $qa, $step, $lines, $context) {
        return $this->textarea($step->get_qt_var($name), $lines, array('readonly' => 'readonly'), $qa);
    }

    public function response_area_input($name, $qa, $step, $lines, $context) {
        $inputname = $qa->get_qt_field_name($name);
        return $this->textarea($step->get_qt_var($name), $lines, array('name' => $inputname), $qa) .
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_externaltask_format_monospaced_renderer extends qtype_externaltask_format_plain_renderer {
    protected function class_name() {
        return 'qtype_externaltask_monospaced';
    }
}
