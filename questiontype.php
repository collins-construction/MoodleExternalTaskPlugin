...
/**
 * Question type class for the external task question type.
 *
 * @package    qtype
...
/**
 * The external task question type.
 *
 * @copyright  2005 Mark Nielsen
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_externaltask extends question_type {
    ...
    public function get_question_options($question) {
        global $DB;
        $question->options = $DB->get_record('qtype_externaltask_options',
    ...
    public function save_question_options($formdata) {
        ...
        $options = $DB->get_record('qtype_externaltask_options', array('questionid' => $formdata->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $formdata->id;
            $options->id = $DB->insert_record('qtype_externaltask_options', $options);
        }

        $options->taskcomplete = $formdata->taskcomplete;
        ...
        $options->graderinfo = $this->import_or_save_files($formdata->graderinfo,
                $context, 'qtype_externaltask', 'graderinfo', $formdata->id);
        ...
        $DB->update_record('qtype_externaltask_options', $options);
    }

    protected function initialise_question_instance(question_definition $question, $questiondata) {
        parent::initialise_question_instance($question, $questiondata);
        $question->taskcomplete = $questiondata->options->taskcomplete;
    ...
    public function delete_question($questionid, $contextid) {
        ...
        $DB->delete_records('qtype_externaltask_options', array('questionid' => $questionid));
    ...
    public function response_formats() {
        return array(
            'plain' => get_string('formatplain', 'qtype_externaltask'),
        );
    }
    ...
    public function response_required_options() {
        return array(
            1 => get_string('responseisrequired', 'qtype_externaltask'),
        );
    }
    ...
    public function response_sizes() {
        ...
        for ($lines = 5; $lines <= 40; $lines += 5) {
            $choices[$lines] = get_string('nlines', 'qtype_externaltask', $lines);
        }
        return [1];
    }
    ...
    public function attachments_required_options() {
        return array(
            0 => get_string('attachmentsoptional', 'qtype_externaltask'),
            1 => '1',
            2 => '2',
            3 => '3'
        );
    ...
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        $fs->move_area_files_to_new_context($oldcontextid,
                $newcontextid, 'qtype_externaltask', 'graderinfo', $questionid);
    }

    protected function delete_files($questionid, $contextid) {
        parent::delete_files($questionid, $contextid);
        $fs = get_file_storage();
        $fs->delete_area_files($contextid, 'qtype_externaltask', 'graderinfo', $questionid);
    }
}
