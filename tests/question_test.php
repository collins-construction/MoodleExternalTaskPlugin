...
/**
 * Unit tests for the externaltask question definition class.
 *
 * @package    qtype
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_externaltask_question_test extends advanced_testcase {
    ...
    public function test_is_complete_response() {
        ...
        $helper = test_question_maker::get_test_helper('externaltask');
        $attachments = array();
        for ($i = 0; $i < 4; ++$i) {
            $attachments[$i] = $helper->make_attachments_saver($i);
        }

        // Create the externaltask question under test.
        $essay = test_question_maker::make_an_essay_question();
        $essay->start_attempt(new question_attempt_step(), 1);
